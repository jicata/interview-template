...
> **Run `gh repo set-default jicata/interview-template` once per clone.** `gh` is fork-aware and resolves the **parent** repo by default, so before this is set a bare `gh issue create` / `gh pr create` targets `prmsolutions/interview-template` — the interviewer's repo. WHY: the whole afk lane runs bare `gh` commands; explicit `--repo` flags are the exception, not the rule. Evidence: 2026-08-26, `gh label create` returned `HTTP 404 .../repos/prmsolutions/interview-template/labels` with no `--repo` flag; `gh repo set-default --view` reported no default set.

> **A `404` on a write to your own repo means the wrong account is active, not a missing resource.** GitHub returns 404 rather than 403 on unauthorized writes so it does not leak whether a private resource exists. Diagnose with `gh api user --jq .login` before believing the error. WHY: the 404 sends you looking for a typo'd repo name when the real cause is identity. Evidence: 2026-08-26 — labels were readable (`length` 10) while `POST /labels` 404'd; `gh auth status` showed `jicata` with `Active account: false`.

> **Check the active `gh` account before any run that writes, and switch without asking.** The operator works across projects under different GitHub accounts, so the active account drifts as a matter of course — this is expected, not a fault. `gh api user --jq .login`; if it is not `jicata`, run `gh auth switch --user jicata` and continue. WHY: reads succeed under the wrong account, so the drift is invisible until the first write, which in a `/ship-*` run is several minutes in. Standing authorization to switch was given 2026-08-26; do not stop to ask. Evidence: observed reverting to `SGalovVSG` mid-session on 2026-08-26.

> **`gh` must be authenticated as `jicata`, not `SGalovVSG`.** Both accounts are logged in (`gh auth status` shows both); `SGalovVSG` has only **READ** on this repo, so every pipeline `gh issue create` / `gh pr create` / `gh pr merge` 403s, and plain `git push` fails with `Permission denied to SGalovVSG`. Fix, once per machine: `gh auth switch --user jicata`. WHY: the failure is intermittent-looking — a cached credential lets some pushes succeed — so it reads as a flaky remote rather than an identity mismatch, and subagents inherit the wrong identity silently. Evidence: verified 2026-08-25 — as `SGalovVSG`, `permissions.push=false`; after switching, `push=true admin=true` and a plain `git push` succeeds. `jicata` is also correct for `jicata/Brochures`.
...
Applies when authoring a PRD (`/write-a-prd`), splitting it (`/prd-to-issues`), or planning a child (`/expand-issue`).

> **Every acceptance criterion describing observable state must name the route that can serve it — and the answer must survive a reload or a selection change.** The Consumer Reality Check proves a consumer exists and what it calls *today*; it does not prove each AC is satisfiable and *stays* satisfied. Walk the ACs one at a time and ask: which endpoint answers this, and does the answer outlive the response that produced it? An AC that holds only while a POST response sits in a `ref` is not satisfied — it is unfalsifiable, and it will be ticked. WHY: PRD #1 specified a **write-only feature**. It named `GET /customers/{id}/reorder-suggestions` and `POST /customers/{id}/draft/lines` and no way to read a draft back, while child issue #4 carried the AC "After a successful save the panel reflects the new draft state". That AC was ticked and merged. It is true for about four seconds, then `ReorderPanel.vue:69` clears `savedDraft` on customer switch and nothing can restore it — the data is committed server-side but the UI is structurally incapable of showing it, which is indistinguishable from not saving. Nobody caught it: not PRD authoring, `/prd-to-issues`, `/expand-issue`, the coder, the reviewer across four passes, or the orchestrator — because every gate checks conformance **to the spec**, and the spec was internally consistent and merely incomplete against reality. Evidence: routes at `6331f04` were `GET /hello`, `GET /customers`, `GET .../reorder-suggestions`, `POST .../draft/lines` — no draft read; operator review 2026-08-26.

> **A PRD's Out of Scope list is a scope boundary, not a design.** Excluding a capability does not specify the affordance that replaces it. PRD #1 excluded "editing the suggested quantity before saving" and named checkboxes as the control — which together silently removed any way for a user to order two packs, a capability the `/expand-issue` plan had explicitly designed in as a cart-style basket merge. WHY: the contradiction surfaced only at review, as dead code, and was then resolved between two agents on document precedence rather than raised with the human. Evidence: PRD #1 Implementation Decisions vs. issue #4's recorded planning note; PR #7 round 1 blocker.
...
> **`arch-frontend.md` A6 gives this repo no styling instruction at all — supply the floor yourself.** A6 routes every styling decision through "the repo's component library" and a design-handoff artifact. This repo has **neither**: `no component library`, no theme, no design pipeline, and not one `.css` file anywhere. Every clause is conditional on infrastructure that is absent, so the section silently no-ops rather than failing loudly, and a slice ships unstyled with no gate able to object. The floor for any component you create: a `<style scoped>` block covering readable spacing, list/table alignment, and visually distinct disabled, empty and error states. Prefer scoped styles over a global stylesheet. WHY: PRD #1's `ReorderPanel.vue` shipped at 169 lines with zero `<style>` and two inline attributes total, and nothing could flag it — the words "style", "CSS", "design" and "visual" appear nowhere in the PRD or any child issue, so Axis A had nothing to grade and Axis B had no rule to cite. This is a demo-facing sandbox, so an unstyled panel is a visible failure even when every test is green. Evidence: `frontend/src/components/ReorderPanel.vue` @ `6331f04`; `arch-frontend.md` A6 and "the design artifact is the build target"; operator review 2026-08-26.
