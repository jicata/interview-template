...
> **Re-verify the active `gh` account at the start of any pipeline run.** The switch persists across shells but was observed reverting to `SGalovVSG` mid-session. Check `gh api user --jq .login` and re-run `gh auth switch --user jicata` if needed. WHY: reads keep working under the wrong account, so the reversion is invisible until the first write — which in a `/ship-*` run is several minutes in. Evidence: 2026-08-26, reverted between the Postman work and issue creation; cause not established.
